...
// Slight modifications were done by Jay Foremska; mostly just changing debugging messages and adding a Pause function
...
void CFMODEngine::Initialize( void )
{
	...
	{
		gEngfuncs.Con_Printf( "[FMOD] Failed to create the FMOD system! (%s)\n", FMOD_ErrorString( m_pResult ) );
		m_iSystemState = 2;
		return;
	}
	...
	if ( m_pResult != FMOD_OK )
	{
		gEngfuncs.Con_Printf( "[FMOD] Failed to initialize the FMOD system! (%s)\n", FMOD_ErrorString( m_pResult ) );
		m_iSystemState = 2;
		return;
	}
...
void CFMODEngine::Update( void )
{
	...
	if ( m_pSoundMusic )
	{
		...
		{
			//gEngfuncs.Con_Printf( "The game is paused! (%f)\n", gHUD.m_flTimeDelta );
			m_pResult = m_pFMODChannelSetPaused( m_pChannelMusic, 1 );
			if( m_pResult != FMOD_OK )
				gEngfuncs.Con_Printf( "[FMOD] Failed to pause music! (%s)\n", FMOD_ErrorString( m_pResult ) );
		}
		else
		{
			//gEngfuncs.Con_Printf( "The game is NOT paused (%f)\n", gHUD.m_flTimeDelta );
			m_pResult = m_pFMODChannelSetPaused( m_pChannelMusic, 0 );
			if( m_pResult != FMOD_OK )
				gEngfuncs.Con_Printf( "[FMOD] Failed to unpause music! (%s)\n", FMOD_ErrorString( m_pResult ) );
		}
...
bool CFMODEngine::InitializeCheckFMODVersion( void )
{
	...
	if ( m_pResult != FMOD_OK )
	{
		gEngfuncs.Con_Printf( "[FMOD] Failed to check FMOD version! (%s)\n", FMOD_ErrorString( m_pResult ) );
		return false;
	}
