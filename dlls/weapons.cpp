...
{
	if ((m_fInReload) && ( m_pPlayer->m_flNextAttack <= gpGlobals->time ) )	// magic nipples - replaced UTIL_WeaponTimeBase()
	{
		// complete the reload. 
...
BOOL CBasePlayerWeapon :: DefaultDeploy( char *szViewModel, char *szWeaponModel, int iAnim, char *szAnimExt, int skiplocal /* = 0 */, int body )
{
	...
	m_pPlayer->m_flNextAttack = gpGlobals->time + 0.5;	// magic nipples - replaced UTIL_WeaponTimeBase()
	m_flTimeWeaponIdle = gpGlobals->time + 1.0;	// magic nipples - replaced UTIL_WeaponTimeBase()
...
BOOL CBasePlayerWeapon :: DefaultReload( int iClipSize, int iAnim, float fDelay, int body )
{
	...
	m_pPlayer->m_flNextAttack = gpGlobals->time + fDelay;	// magic nipples - replaced UTIL_WeaponTimeBase()
